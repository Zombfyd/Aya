...
// Import the play attempts service
...
if (import.meta.env.DEV) {
  window.grantPlayAttempts = async (wallet, quantity = 10) => {
    if (!wallet) {
    ...
    try {
      const result = await playAttemptsService.localGrantPlayAttempts(wallet, quantity);
      console.log('Successfully granted play attempts:', result);
      alert(`Granted ${quantity} play attempts to ${wallet}`);
      return result;
    } catch (error) {
      console.error('Failed to grant play attempts:', error);
...
const GameApp = () => {
  ...
  const [currentPlayAttempt, setCurrentPlayAttempt] = useState(null);
  const [showPlayAttemptsPurchase, setShowPlayAttemptsPurchase] = useState(false);
  const [ayaBalance, setAyaBalance] = useState(BigInt(0));
  ...
  const checkTokenBalance = async (tokenType) => {
    try {
      if (!wallet.connected) {
        logger.log(`Wallet not connected, returning 0 balance for ${tokenType}`);
        return BigInt(0);
      }

      logger.log(`Checking balance for ${tokenType} at address ${wallet.account.address}`);
      const { data: coins } = await client.getCoins({
        owner: wallet.account.address,
        coinType: tokenType
      });

      logger.log(`Found ${coins.length} coins for ${tokenType}:`, coins);
      const total = coins.reduce((total, coin) => total + BigInt(coin.balance), BigInt(0));
      logger.log(`Total balance for ${tokenType}: ${total.toString()}`);
      return total;
    } catch (error) {
    ...
    const updateWalletState = async () => {
      if (wallet.connected && wallet.account) {
        ...
        try {
          ...
          // Also update the ayaBalance state directly
          setAyaBalance(ayaBalance);
    ...
    const setupGameOver = (manager, gameType) => {
        if (manager) {
            manager.onGameOver = async (finalScore) => {
                ...
                try {
                    // Complete play attempt if in paid mode
                    if (gameMode === 'paid' && currentPlayAttempt) {
                        await completeCurrentPlayAttempt(finalScore);
                    }
...
}, [gameMode, wallet.connected, handleScoreSubmit, paymentStatus, paidGameAttempts, maxAttempts, currentPlayAttempt]);
...
const handleSuinsChange = (e) => {
  ...
  const fetchPlayAttempts = useCallback(async () => {
    ...
    try {
      ...
      const stats = await playAttemptsService.getPlayerStats(wallet.account.address);
      
      // Calculate remaining play attempts
      const remainingAttempts = stats.totalAttempts - stats.completedAttempts;
      setPlayAttempts(remainingAttempts);
      
      logger.log('Fetched play attempts:', {
        total: stats.totalAttempts,
        completed: stats.completedAttempts,
        remaining: remainingAttempts
      });
    } catch (error) {
      logger.error('Error fetching play attempts:', error);
    } finally {
  ...
  const completeCurrentPlayAttempt = async (score, completed = true) => {
    if (!currentPlayAttempt || !currentPlayAttempt.id) return;
    
    try {
      const result = await playAttemptsService.completePlayAttempt(
        currentPlayAttempt.id,
        score > 0, // score submitted?
        {
          finalScore: score,
      ...
      logger.log('Completed play attempt:', result);
      setCurrentPlayAttempt(null);
      
      // Refresh play attempts count
      fetchPlayAttempts();
    } catch (error) {
      logger.error('Error completing play attempt:', error);
    }
  };

  // Add function to handle play attempts purchase
  const handlePlayAttemptsPurchase = async (purchaseDetails) => {
    try {
      logger.log('Starting play attempts purchase:', purchaseDetails);
      setPaying(true);
      setTransactionInProgress(true);
      
      const { plays, paymentToken, isAyaPayment, cost } = purchaseDetails;
  ...
  const renderPlayAttemptsCounter = () => {
    ...
    return (
      <div className="play-attempts-counter">
        <div>
          Play Attempts: 
          <span className="attempts-count"> {isLoadingAttempts ? '...' : playAttempts}</span>
          {isLoadingAttempts && <span className="attempts-loading"></span>}
        </div>
        <button
          className="buy-attempts-button"
          onClick={() => {
            console.log("Buy More button clicked!");
            console.log("Before toggle, showPlayAttemptsPurchase =", showPlayAttemptsPurchase);
            // Toggle the state instead of just setting to true
            setShowPlayAttemptsPurchase(!showPlayAttemptsPurchase);
            setTimeout(() => {
              console.log("After toggle, showPlayAttemptsPurchase =", !showPlayAttemptsPurchase);
            }, 100);
          }}
  ...
  return (
    <div className={`game-container ${gameState.gameStarted ? 'active' : ''}`}>
      ...
      {/* Play Attempts Purchase Modal */}
      {console.log("Rendering PlayAttemptsPurchase with showPlayAttemptsPurchase =", showPlayAttemptsPurchase)}
      {console.log("Current AYA balance:", ayaBalance.toString(), "- Type:", typeof ayaBalance)}
      {console.log("Current SUI balance:", balance.toString(), "- Type:", typeof balance)}
      <PlayAttemptsPurchase
        isOpen={showPlayAttemptsPurchase}
        onClose={() => {
          console.log("Modal onClose triggered");
          setShowPlayAttemptsPurchase(false);
        }}
        onPurchase={handlePlayAttemptsPurchase}
        isNFTVerified={isNFTVerified}
        balance={balance}
        ayaBalance={ayaBalance}
        maxAllowedAttempts={1000}
        remainingAttempts={playAttempts}
